#!/usr/bin/env python3
"""Transfer Learning pipeline using EfficientNet-B0 (Fine-Tuning Phase)."""
...
def main():
    ...
    CHECKPOINT_WARM = f"{RESULTS_DIR}/efficientnet_frozen_{PAIR_KEY}.pth"
    CHECKPOINT_FT   = f"{RESULTS_DIR}/efficientnet_finetuned_{PAIR_KEY}.pth"
    LOG_FILE_FT     = f"{RESULTS_DIR}/training_finetune_{PAIR_KEY}.log"
    ...
    logging.basicConfig(
        ...
        datefmt="%Y-%m-%d %H:%M:%S",
        handlers=[
            logging.FileHandler(LOG_FILE_FT, mode="w", encoding="utf-8"),
            logging.StreamHandler()
        ]
    )
    log = logging.getLogger()
    
    log.info(f"Starting EfficientNet Fine-Tuning for pair: {PAIR_KEY}")
    ...
    log.info(f"Loading warm-up checkpoint from {CHECKPOINT_WARM}...")
    try:
        model.load_state_dict(torch.load(CHECKPOINT_WARM, map_location=DEVICE))
    except Exception as e:
    ...
    log.info("=" * 65)
    log.info("FINE-TUNING EVALUATION")
    log.info("=" * 65)
    ...
    ax.set_title(f"EfficientNet Fine-Tuned ({PAIR_KEY})")
    fig.tight_layout()
    confusion_path = os.path.join(RESULTS_DIR, f"efficientnet_finetuned_confusion_matrix_{PAIR_KEY}.png")
    fig.savefig(confusion_path, dpi=200)
    plt.close(fig)
    
    log.info(f"Log saved to: {LOG_FILE_FT}")
